...
function AddPlanModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-lg mx-4 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-slate-900">Thêm kế hoạch HC</h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Venture</label>
            <select
              value={ventureId}
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1.5">Planned HC</label>
            <input
              type="number"
              min={1}
              value={plannedHc}
...
function EditPlanModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-2xl border border-slate-200 shadow-xl w-full max-w-lg mx-4 p-6">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-lg font-semibold text-slate-900">Chỉnh sửa kế hoạch HC</h3>
          <button
            onClick={onClose}
            className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-lg cursor-pointer transition-colors"
          >
    ...
    return (
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h2 className="text-2xl font-bold text-slate-900">HC Plans</h2>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-slate-500">Total Planned HC</span>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-slate-500">Used HC</span>
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 hover:shadow-md transition-shadow">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-medium text-slate-500">Available HC</span>
              <div className="w-10 h-10 bg-emerald-50 rounded-xl flex items-center justify-center">
                <BarChart3 className="w-5 h-5 text-emerald-600" />
              </div>
            </div>
            <div className="text-3xl font-bold text-slate-900">{totalAvailable}</div>
          </div>
        </div>
        ...
        {summary && summary.ventures.length > 0 ? (
                ...
                >
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-sm font-semibold text-slate-900">{venture.ventureName}</h3>
                        <span className="inline-block text-xs px-2 py-0.5 bg-slate-100 text-slate-600 rounded-full font-medium mt-0.5">
                          {venture.ventureCode}
                        </span>
                      </div>
                    </div>
                        ...
                        return (
                          <div key={deptName}>
                            <div className="flex items-center justify-between text-xs mb-1">
                              <span className="text-slate-600 truncate mr-2">{deptName}</span>
          ...
          !loading && (
            <div className="bg-white rounded-2xl border border-slate-200 p-12 text-center">
              ...
              <h3 className="text-lg font-medium text-slate-900 mb-1">Chưa có kế hoạch HC nào</h3>
              <p className="text-slate-500 text-sm">Hãy thêm kế hoạch headcount đầu tiên</p>
              <button
                onClick={() => setShowAddModal(true)}
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <h2 className="text-2xl font-bold text-slate-900">{selectedVentureName}</h2>
          <p className="text-slate-500 text-sm mt-1">
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {loading ? (
          ...
          <div className="p-12 text-center">
            <BarChart3 className="w-12 h-12 text-slate-300 mx-auto mb-3" />
            <h3 className="text-lg font-medium text-slate-900 mb-1">Chưa có kế hoạch nào</h3>
            <p className="text-slate-500 text-sm">Venture này chưa có kế hoạch HC</p>
            <button
              onClick={() => setShowAddModal(true)}
              className="mt-4 flex items-center gap-2 px-4 py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-xl cursor-pointer mx-auto transition-colors"
            >
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-100 bg-slate-50/50">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase min-w-[160px]">
                    Usage
                  </th>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <button
                            ...
                            className="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg cursor-pointer transition-colors disabled:opacity-50"
                            title="Xóa"
                          >
